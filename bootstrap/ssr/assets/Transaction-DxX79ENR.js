import { ref, computed, onMounted, onUnmounted, watch, withCtx, unref, createVNode, toDisplayString, createTextVNode, openBlock, createBlock, createCommentVNode, Fragment, renderList, Transition, withDirectives, vModelText, useSSRContext } from "vue";
...
import { f as formatIDR, s as showImage } from "./utils-DIF4pdrF.js";
      ...
      _push(ssrRenderComponent(GuestLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            ...
            )}"${_scopeId}><svg class="icon icon-dana d-inline rounded-circle border-primary border" style="${ssrRenderStyle({ "width": "1rem", "height": "1rem", "padding": "0.1rem" })}"${_scopeId}><rect width="24" height="24" fill="#ccc" rx="4"${_scopeId}></rect></svg> ${ssrInterpolate("Dana")}</span></div></div></div><div class="d-flex mt-3"${_scopeId}><button type="button" data-bs-toggle="collapse" data-bs-target=".multi-collapse" aria-controls="paymentGuideCollapseCash paymentGuideCollapseDana" class="btn btn-outline-secondary btn-sm py-0 w-100 border"${_scopeId}><span style="${ssrRenderStyle({ "font-size": "0.8rem" })}"${_scopeId}>${ssrInterpolate("Payment Guide")}</span></button></div>`);
...
//# sourceMappingURL=Transaction-DxX79ENR.js.map
