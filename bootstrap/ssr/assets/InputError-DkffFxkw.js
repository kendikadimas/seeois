    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (Array.isArray(__props.message)) {
        ...
        ssrRenderList(__props.message, (error) => {
          _push(`<p class="text-danger mb-1" style="${ssrRenderStyle({ "font-size": "smaller" })}">${ssrInterpolate(error)}</p>`);
        });
        _push(`<!--]-->`);
      } else {
        _push(`<!---->`);
      ...
      if (!Array.isArray(__props.message)) {
        _push(`<p class="text-danger mb-1" style="${ssrRenderStyle({ "font-size": "smaller" })}">${ssrInterpolate(__props.message)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
